...
import { uploadToCloudinary, deleteFromCloudinary } from "../../utils/cloudConfig.js"
...
const completeProfile = async (req, res, next) => {
  try {
    ...
    return res
      .status(200)
      .json(new ApiResponse(200, existingUser, "Profile completed successfully"))
  } catch (error) {
    next(error)
  }
}
...
const uploadOrUpdateProfilePicture = async (req, res, next) => {
  try {
    ...
    if (existingUser.profile_pic_url?.publicId) {
      await deleteFromCloudinary(existingUser.profile_pic_url.publicId)
    }
...
const updateProfile = async (req, res, next) => {
  try {
    ...
    const { address, city, fatherName, phone, dob } = req.body
...
const getMembershipStatus = async (req, res, next) => {
  try {
    ...
    if (!userMembership || userMembership.status !== "ACTIVE") {
      return res
        .status(200)
        .json(new ApiResponse(200, null, "No active membership found"))
    }
